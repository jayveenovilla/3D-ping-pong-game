...
public class SpawnBonus : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if(time <= 0)
        {
            ...
            if(prefabCount >= 0 && prefabCount <= 4 && GameManager._instance.player.penaltyCount < 1)        //blackstar spawns are penalties
            {
    ...
    private IEnumerator blackStarCountdown(GameObject go)
    {
        ...
        if (go != null)
        {
            StartCoroutine(myParticleEffects.blackFirework(go.transform));
            Destroy(go);
            GameManager._instance.player.penaltyCount--;
        }
    }
